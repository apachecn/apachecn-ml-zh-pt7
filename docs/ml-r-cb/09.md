...
10.  You now need to add the bootstrap action with `Custom action`, and add `s3://<location>/bootstrapRHadoop.sh` within the S3 location.![How to do it...](img/00277.jpeg)
    ...
    在这个菜谱中，我们演示了如何在 Amazon EMR 上设置 RHadoop。这样做的好处是，您只需在几分钟内点击几下鼠标，就可以快速创建可扩展的按需 Hadoop。这有助于节省构建和部署 Hadoop 应用程序的时间。但是，您必须为每个实例的运行小时数付费。在使用 Amazon EMR 之前，您应该创建一个 AWS 帐户，并知道如何设置 EC2 密钥对和 S3。然后，您可以开始在 Amazon EMR 上安装 RHadoop。
