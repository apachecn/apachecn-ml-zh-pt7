...
有了准备好的数据，我们就可以构建模型了。出于演示的目的，我们将为这些应用程序选择一个相当标准的架构:LSTM 层的组合，通过全局池化和丢弃进行标准化，在顶部有一个密集层。为了实现真正有竞赛力的解决方案，需要对架构进行一些调整:更“重”的模型、更大的嵌入、LSTM 层中更多的单元，等等。
...
我们现在已经展示了一个解决情感归属问题的示例管道(识别文本中导致注释者对情感分类做出决定的部分)。如果您想要实现有竞赛力的绩效，可以进行一些改进，下面按可能的影响顺序列出:
