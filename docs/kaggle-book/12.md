...
*我通过执行 EDA(探索性数据分析)，创建本地验证，构建一些简单的模型，并提交给 Kaggle 以获得排名分数，来开始每场比赛。这培养了一种直觉，即为了建立一个准确且有竞赛力的模型，需要做什么。*
...
在我看来，没有经验的 Kagglers 经常忽视本地验证的重要性。看到你的名字出现在排名上令人激动。我们很容易专注于提高我们的排名分数，而不是交叉验证分数。

你在过去的比赛中犯过什么错误？

*很多次，我犯了这样的错误:相信排名分数而不是交叉验证分数，并选择了错误的最终提交。*
...
我认为许多参加比赛的初学者被公共排名所诱惑，在没有一个好的验证策略的情况下建立他们的模型。当在排名上衡量他们的成功时，他们可能会过度适应公共测试数据。比赛结束后，他们的模型并不能够归纳到看不见的私人测试数据中，他们经常会跌出数百个名次。我仍然记得在梅赛德斯-奔驰绿色制造*比赛期间，我是多么沮丧，因为我没能登上公众排名。但是当最后的积分榜出来的时候，有多少人在排名上上下洗牌，这是一个很大的惊喜。从那以后，我一直牢记，一个合适的验证方案对于应对欠拟合和过拟合的挑战是非常重要的。*
