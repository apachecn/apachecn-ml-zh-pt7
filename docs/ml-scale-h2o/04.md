...
在我们的基本工作流程中，我们只研究了一种类型的模型(XGBoost ),同时只改变了几个默认参数。H2O-3(及其苏打水扩展)有一个监督和非监督学习算法的广泛列表，以及一系列广泛的参数和超参数来设置您的规格。此外，这些算法可以有力地结合到 AutoML 工作流中，该工作流探索多个模型和超参数空间，并在排行榜上排列产生的最佳模型。您还可以控制交叉验证技术、检查点、再训练和再现性。
