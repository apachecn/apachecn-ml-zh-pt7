...
异常检测的第二个用例是日志分析。许多软件应用程序会生成大量日志，其中包含有关程序执行的所有类型的信息。这些日志通常被临时或长期存储，以供进一步分析。在某些情况下，这些分析可能是手动搜索软件中某一点发生的特定信息，但在其他时候，它们可能是自动日志处理程序。
...
现在让我们应用使用 River 的一级 SVM 的在线培训流程。
