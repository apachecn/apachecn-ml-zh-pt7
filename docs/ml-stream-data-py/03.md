...
*   **Spark Streaming**:Spark Streaming是 Spark 的补充，是当今大数据的主要工具之一。Spark Streaming 可以插入 Kafka、Flume 和 Amazon Kinesis 等源，从而使流式数据可以在 Spark 环境中访问。
...
*   **Apache Storm**:Apache Storm是一个流处理计算框架，允许进行分布式计算。它用于使用 Hadoop 实时处理流式数据。
