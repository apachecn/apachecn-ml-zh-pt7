...
6.  训练结束后，H2O 汽车公司应该已经训练了一些车型，并根据排行榜上的默认排名性能指标对它们进行了排名。排行榜上排名最高的型号称为*领先者*，可以通过使用`aml.leader`命令直接访问。使用这个引用，您可以通过运行以下命令将 leader 模型下载为 POJO:
...
5.  一旦训练结束，你有了排行榜，你就可以使用`aml@leaderboard`进入领导者模式。我们还可以通过执行以下命令将 leader 模型下载为 POJO:
