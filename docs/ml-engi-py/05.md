...
就基础设施而言，这可能非常依赖于您用于培训的模型和数据。如果您要对具有三个要素的数据进行线性回归训练，并且您的数据集仅包含 10，000 条表格记录，那么您可能无需太多考虑就可以在笔记本规模的硬件上运行此操作。这不是很多数据，而且你的模型没有很多自由参数。如果您在一个更大的数据集上进行训练，比如包含 1 亿条表格记录的数据集，那么您可以从 Spark 集群之类的并行化中受益。然而，如果你在 1000 张图像上训练一个 100 层深度卷积神经网络，那么你可能会想要使用 GPU。有很多选择，但关键是为工作选择正确的东西。
